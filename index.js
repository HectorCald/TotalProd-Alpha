...
app.get('/obtener-orden-produccion', requireAuth, async (req, res) => {
    const { spreadsheetId } = req.user;
    ...
    try {
        ...
        const response = await sheets.spreadsheets.values.get({
            spreadsheetId,
            range: 'Orden produccion!A2:P' // Columnas A a G para todos los campos
        });

        const rows = response.data.values || [];
        const ordenes = rows.map(row => ({
            id: row[0] || '',                    // ID
            fecha_emision: row[1] || '',                 // FECHA
            etapa: row[2] || '',              // PRODUCTO
            destino: row[3] || '',           // HORA-INICIO
            idProducto: row[4] || '',              // HORA-FIN
            producto: row[5] || '',        // PROCEDIMIENTOS
            gramaje: row[6] || '',        // OPERADOR
            cantidad: row[7] || '',              // OPERADOR
            fecha_materiaPrima: row[8] || '',              // HORA-FIN
            kilos: row[9] || '',        // PROCEDIMIENTOS
            obs_materiaPrima: row[10] || '',              // OPERADOR
            fechaRegistro: row[11] || '',          // OBSERVACIONES
            idProduccion: row[12] || '', // ID PRODUCCIÓN
            fecha_ingreso: row[13] || '', // FECHA INGRESO
            idIngreso: row[14] || '', // ID INGRESO
            operador: row[15] || '' // OPERADOR
        }));
        ...
        console.error('Error al obtener las ordenes de producción:', error);
        res.status(500).json({
            success: false,
            error: 'Error al obtener las ordenes de producción'
        });
    }
});
app.post('/registrar-orden-produccion', requireAuth, async (req, res) => {
    try {
        const { spreadsheetId } = req.user;
        const { idProducto, producto, gramos, cantidad, destino, operador } = req.body;
        ...
        const response = await sheets.spreadsheets.values.get({
            spreadsheetId,
            range: 'Orden produccion!A2:A'
        });

        const rows = response.data.values || [];
        const lastId = rows.length > 0 ?
            Math.max(...rows.map(row => parseInt(row[0].split('-')[1]))) : 0;
        const newId = `ORD-${(lastId + 1).toString().padStart(3, '0')}`;
...
app.delete('/eliminar-orden-produccion/:id', requireAuth, async (req, res) => {
    try {
        const { spreadsheetId } = req.user;
        const { id } = req.params;
        ...
        const tareasSheet = spreadsheet.data.sheets.find(
            sheet => sheet.properties.title === 'Orden produccion'
        );

        if (!tareasSheet) {
            return res.status(404).json({
                success: false,
                error: 'Hoja de ordenes no encontrada'
            });
        }
...
app.put('/editar-orden-produccion/:id', requireAuth, async (req, res) => {
    try {
        const { spreadsheetId } = req.user;
        const { id } = req.params;
        const { idProducto, producto, gramos, cantidad, destino, operador } = req.body;
        ...
        // Preparar fila actualizada
        const updatedRow = [
            ...rows[rowIndex].slice(0, 3), // Mantener datos hasta etapa
            destino,                // Actualizar destino
            idProducto || rowIndex[4], // Actualizar idProducto
            producto,                // Actualizar producto
            gramos,                // Actualizar gramos
            cantidad,                // Actualizar cantidad
            rows[rowIndex][6],            // Mantener fecha_materiaPrima
            rows[rowIndex][7],            // Mantener kilos
            rows[rowIndex][8],            // Mantener obs_materiaPrima
            rows[rowIndex][9],            // Mantener fechaRegistro
            rows[rowIndex][10],           // Mantener idProduccion
            rows[rowIndex][11],           // Mantener fecha_ingreso
            rows[rowIndex][12],           // Mantener idIngreso
            operador                  // Actualizar operador
        ];

        // Actualizar la fila
        await sheets.spreadsheets.values.update({
            spreadsheetId,
            range: `Orden produccion!A${rowIndex + 2}:P${rowIndex + 2}`,
            valueInputOption: 'USER_ENTERED',
            resource: {
                values: [updatedRow]
            }
...
app.put('/cambio-etapa-produccion/:id', requireAuth, async (req, res) => {
    try {
        const { spreadsheetId } = req.user;
        const { id } = req.params;
        ...
        // Preparar fila actualizada
        const updatedRow = [
            ...rows[rowIndex].slice(0, 2), // Mantener datos hasta fecha_emision
            etapa,                // Actualizar etapa
        ];

        // Actualizar la fila
        await sheets.spreadsheets.values.update({
            spreadsheetId,
            range: `Orden produccion!A${rowIndex + 2}:C${rowIndex + 2}`,
            valueInputOption: 'USER_ENTERED',
            resource: {
                values: [updatedRow]
            }
        ...
        res.json({
            success: true,
            message: 'Etapa actulizada correctamente'
        });
...
app.put('/rellenar-produccion/:id', requireAuth, async (req, res) => {
    try {
        const { spreadsheetId } = req.user;
        const { id } = req.params;
        const { idRegistro, etapa } = req.body;
        ...
        const updatedRow = [
            ...rows[rowIndex].slice(0, 2), // Mantener datos hasta fecha_emision
            etapa,                // Actualizar etapa
            rows[rowIndex][3],            // Mantener destino
            rows[rowIndex][4],            // Mantener idProducto
            rows[rowIndex][5],            // Mantener producto
            rows[rowIndex][6],            // Mantener gramos
            rows[rowIndex][7],            // Mantener cantidad
            rows[rowIndex][8],            // Mantener fecha_materiaPrima
            rows[rowIndex][9],            // Mantener kilos
            rows[rowIndex][10],           // Mantener obs_materiaPrima
            fecha,              // Actualizar fechaRegistro
            idRegistro,        // Actualizar idProduccion
        ];

        // Actualizar la fila
        await sheets.spreadsheets.values.update({
            spreadsheetId,
            range: `Orden produccion!A${rowIndex + 2}:M${rowIndex + 2}`,
            valueInputOption: 'USER_ENTERED',
            resource: {
                values: [updatedRow]
            }
        ...
        console.error('Error al actulizar registro en orden', error);
        res.status(500).json({
            success: false,
...
app.put('/entregar-orden-mp/:id', requireAuth, async (req, res) => {
    try {
        const { spreadsheetId } = req.user;
        const { id } = req.params;
        ...
        // Preparar fila actualizada
        const updatedRow = [
            ...rows[rowIndex].slice(0, 8), // Mantener datos hasta cantidad
            fecha,                // Actualizar fecha_materiaPrima
            peso,                // Actualizar kilos
            observaciones || '', // Actualizar obs_materiaPrima
        ];

        // Actualizar la fila
        await sheets.spreadsheets.values.update({
            spreadsheetId,
            range: `Orden produccion!A${rowIndex + 2}:K${rowIndex + 2}`,
            valueInputOption: 'USER_ENTERED',
            resource: {
                values: [updatedRow]
            }
        ...
        console.error('Error al realizar la entrega de materia prima: ', error);
        res.status(500).json({
            success: false,
            error: 'Error al realizar la entrega de materia prima'
        });
    }
});
