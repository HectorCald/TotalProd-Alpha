...
async function mostrarOpcionesCatalogo() {
    ...
    const view = document.querySelector('.catalogos-cont');
    const botonesPrecios = precios.map(precio => `
        <button class="btn-catalogo-generar btn-precio" style="max-width: 100% !important" data-precio="${precio.precio}">
        ...
        <div class="top-view">
            <div class="encabezado">
                <div class="titulo-back">
                    <p class="titulo">Catalogos</p>
                </div>
            </div>
        </div>
        <div class="contenido-view">
            <p class="subtitulo">Selecciona un catalogo para generar el PDF</p>
    ...
    const botones = view.querySelectorAll('.btn-precio');
...
async function generarCatalogo(tipoPrecio) {
    ...
    try {
        ...
        mostrarNotificacion('Error al generar el catalogo',{tipo:'error'})
    } finally {
